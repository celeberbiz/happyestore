...
<div class="col-md-9">
			...
			<div class="items">
				@foreach($products as $product)
					<div class="item col-md-3 col-sm-4 col-xs-12 clickable" data-href="{{ url('/product/'.$product->id.'-'.Str::slug($product->name).'/show') }}">
						<div class="photo"><img src="{{ url($product->thumbnail) }}">{!! $product->created_at >= Carbon\Carbon::now()->subweek() ? '<span class="new-product">NEW</span>' : '' !!}</div>
	...
	{!! str_replace('/?', '?', $products->appends(['sort' => Request::get('sort')])->render()) !!}
</div>
</div>
</div>
@stop